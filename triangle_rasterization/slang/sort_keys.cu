...
// It is licensed under the Apache 2.0 license, which has been reproduced below.
...
std::tuple<torch::Tensor, torch::Tensor> sort_keys(
  ...
  at::Tensor keys_sorted = torch::zeros(keys.sizes(), keys.options());
  at::Tensor values_sorted = torch::zeros(values.sizes(), values.options());
  ...
  at::Tensor workspace = torch::empty(
      ...
      keys.options().dtype(torch::kUInt8)
  );
